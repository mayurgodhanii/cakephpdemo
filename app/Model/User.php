...
class User extends AppModel {
    ...
    public $hasOne=array(           
        'UserDetail' => array(
            ...
            'condition' => array('UserDetail.user_id=User.id')
        ),
        'UserAddress' => array(
            'className' => 'UserAddress',
            'condition' => array('UserAddress.user_id=User.id')
        ),
...
//        $this->_setupValidation();
    ...
    public function confirmPassword($password = null) {
        if ((isset($this->data[$this->alias]['password']) && isset($password['cpassword'])) && !empty($password['cpassword']) && ($this->data[$this->alias]['password'] === $password['cpassword'])) {
            return true;
        }
        return false;
    }
    ...
    public function checkEmailVerfificationToken($token = null) {
        $result = $this->find('first', array(
            ...
            'fields' => array(
                'id', 'email', 'email_token_expires', 'role_id')));
    ...
    public function changePassword($postData = array()) {
        ...
        if ($this->validates()) {
            $this->data[$this->alias]['password'] = $this->hash($this->data[$this->alias]['new_password'], null, true);
            $this->data[$this->alias]['textpassword'] = $this->data[$this->alias]['new_password'];
    ...
    public function register($postData = array(), $options = array()) {
        $Event = new CakeEvent(
                        'Users.Model.User.beforeRegister', $this, array(
                    'data' => $postData,
                    'options' => $options
                        )
        );
        ...
        if ($this->validates()) {
            ...
            $Event = new CakeEvent(
                            'Users.Model.User.afterRegister', $this, array(
                        'data' => $this->data,
                        'options' => $options
                            )
            );
            ...
            switch ($by) {
                case 'username':
                    $results['conditions'] = Set::merge(
                                    $query['conditions'], array($this->alias . '.username LIKE' => $like));
                    break;
                case 'email':
                    $results['conditions'] = Set::merge(
                                    $query['conditions'], array($this->alias . '.email LIKE' => $like));
                    break;
                case 'any':
                    $results['conditions'] = Set::merge(
    ...
    public function add($postData = null) {
        if (!empty($postData)) {
            ...
            if ($this->validates()) {
//                if (empty($postData[$this->alias]['role_id'])) {
...
//                }
                //$postData[$this->alias]['password'] = $this->hash($postData[$this->alias]['password'], 'sha1', true);
                $this->create();
                $result = $this->saveAssociated($postData);
    ...
    public function checkPassword($password = NULL) {
        ...
        } else if (isset($password['new_password'])) {
    ...
    public function userResetPassword($id = null) {
        ...
        if (!empty($data)) {
            ...
            $userData['User']['email_verified'] = $data['User']['email_verified'];
            $userData['User']['active'] = $data['User']['active'];
            $userData['User']['role_id'] = $data['User']['role_id'];
            $userData['User']['password'] = AuthComponent::password($random_password);
            $this->save($userData, FALSE);

            $mailer_to = $data['User']['email'];
            $mailer_subject = "Password Reset : " . Configure::read('SITE_TEAM');
            $mailer_from = Configure::read('FROM_EMAIL_ADDRESS');
            ...
            $mailcontent['mailer_to'] = $mailer_to;
            $mailcontent['mailer_subject'] = $mailer_subject;
            $mailcontent['mailer_from'] = $mailer_from;
    ...
    function deleteUser($id) {
        ...
        return ($user['User']['is_delete'] == 1) ? 0 : 1;
    }             
    

    function emailverification($id) {
        $this->id = $id;
        $this->saveField('email_verified', 1);
        return true;
    }
  ...
  /*  function deleteuser($id) {
    ...
    function adduser($data) {
        ...
        $data['UserDetail']['user_id'] = $last_insert_id;
        if (!empty($data['UserDetail']['image'])) {
            $data['UserDetail']['image'] = $this->__uplaodfiles($data['UserDetail']['image'], "upload/");
        } else {
            $data['UserDetail']['image'] = "defalut_avtar.png";
        }
